...
void isIntegerValid(string&, const int); 
bool isOnlyDigits(string);  
void EmptyArray(string[], int);
void MakeSpaces(int);  
...
string arrayToString(LongInteger, const int);  
int SumAlignment(LongInteger[], LongInteger&);
int SmallestInt(const LongInteger, const LongInteger); 
LongInteger add(const LongInteger, const LongInteger, const int); 
LongInteger multiply(const LongInteger, const LongInteger, LongInteger[12], const int); 
...
int main()
{
    ...
    cout << "Welcome!\nThis program adds between 2 and 13 very large numbers accurately.\nIt multiplies the first and last numbers entered and shows the intermediate steps of the product. Enjoy!!\n\n"; 
    ...
    while(will_continue)
    {
        ...
        for (int i = 0; i < num_of_integers; i++)
        {
            ...
            isIntegerValid(integer_input, MAX_DIGITS); //check for valid input of Integer
            integer_List[i].stringToArray(integer_input); //input valid integer into LongInteger array
        }
  
        //call multiplication function 
        product_integer = multiply(integer_List[0], 
                                   integer_List[num_of_integers - 1], 
                                   interim_products, ARRAY_SIZE);
        ...
        //smallest_int_size = amount of numbers for intermediate step
        ...
        //get size of horizonal line for formatting based on sum_integer with commas
        horizonal_line_size = sum_integer.IntSize() + 3;
        cout << "\nThe sum of:\n\n"; 
      
        //formating for sum display
        for (int i = 0; i < num_of_integers; i++)
        { 
        ...
        //smallest_int_size = amount of numbers for intermediate step
        //horizonal line size is now based on length of product_integer with commas
        horizonal_line_size = product_integer.IntSize() + 3;
        cout << "\nThe product of: \n" << endl;
      
        // make the nessesary spaces so everything lines up for product display
        ...
        //go to next line for intermediate addition steps
        cout << endl;
  
        //print out intermediate addition steps
...
{
    while ((stringToIntMain(size) < lowerBound || 
           stringToIntMain(size) > upperBound) || 
                          (!spacesCheck(size)) || 
                          size.find('.') != -1 || 
                             size.length() > 12 )
    {
        ...
        {
            cout << "\nThe input was not a number between " << 
                     lowerBound << 
                     " and " << 
...
{
    int biggest_integer_size = 0; 
    int temp_sum_value = 0;
...
LongInteger multiply(const LongInteger first_integer, const LongInteger last_integer, LongInteger products[12], const int ARRAY_SIZE)
{
    ...
    if(str_first_integer.size() <= str_last_integer.size())
    {
        ...
        for (int i = str_first_integer.size() - 1; i >= 0; i--) // TODO: reverse loop
        {
            ...
            {
                if (int(str_first_integer[i] - '0') * 
                    int(str_last_integer[j] - '0') + 
                    carried_value < 10)
                {
                    product_value = int(str_first_integer[i] - '0') * 
                                    int(str_last_integer[j] - '0') + 
                                    carried_value;
    ...
    else
    {
        ...
        for (int i = str_last_integer.size() - 1; i >= 0; i--)
        {
            ...
            {
                if (int(str_first_integer[j] - '0') * 
                    int(str_last_integer[i] - '0') + 
                    carried_value < 10)
                  {
                      product_value = int(str_first_integer[j] - '0') * 
                                      int(str_last_integer[i] - '0') + 
                                      carried_value;
...
{
    while (!isOnlyDigits(int_input) || 
            !spacesCheck(int_input) || 
           !leadWithZero(int_input) || 
    int_input.length() > MAX_DIGITS || 
                      int_input == "")
    {
